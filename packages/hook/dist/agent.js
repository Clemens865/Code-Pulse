// `claude-pulse-team agent` — long-running drain loop.
...
export async function agent(opts = {}) {
    ...
    const log = opts.quiet ? () => { } : (msg) => console.log(`[agent] ${msg}`);
    let waitMs = minMs;
    let stopping = false;
    const stop = () => {
        if (stopping)
    ...
    while (!stopping) {
        ...
        if (before > 0) {
            try {
                ...
                if (!opts.quiet) {
                    log(`drained · accepted=${r.accepted} dup=${r.duplicates} rej=${r.rejected} err=${r.errors} pending=${r.pending}`);
                }
