...
import { markRejected, markRetry, markSynced, pendingCount, requeueQuarantined, takeBatch, } from "./outbox.js";
...
function setCooldown(on) {
    try {
        ...
        else if (existsSync(COOLDOWN_PATH))
            unlinkSync(COOLDOWN_PATH);
    }
    catch {
        // best-effort
    }
...
export async function sync(opts = {}) {
    ...
    if (opts.requeueQuarantined) {
        ...
        if (!opts.quiet)
            console.error(`[sync] requeued ${requeued} quarantined row(s)`);
    }
    else if (inCooldown()) {
