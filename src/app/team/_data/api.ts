...
export const api = {
  ...
  mergeProjects: (winnerId: string, loserIds: string[]) =>
    post<{ ok: boolean; winner_id: string; merged: string[]; repointed: Record<string, number> }>(
      "/v1/admin/projects/merge",
      { winner_id: winnerId, loser_ids: loserIds },
    ),
  auditLog: () =>
    get<{
      entries: Array<{
  ...
  deadLetter: (opts: { limit?: number; includeResolved?: boolean } = {}) => {
    const params = new URLSearchParams();
