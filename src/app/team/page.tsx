...
export default function OverviewPage() {
  ...
  const [dlqOpen, setDlqOpen] = useState<number>(0);
  ...
  // Open DLQ count — derive failures the operator should investigate. Silent
  // by default (no badge when 0) so it doesn't add noise during healthy ops.
  useEffect(() => {
    if (source !== "live") {
      setDlqOpen(0);
      return;
    ...
    let cancelled = false;
    api
      .deadLetter({ includeResolved: false })
      .then((r) => {
        if (!cancelled) setDlqOpen(r.events.length);
      })
      .catch(() => {
        if (!cancelled) setDlqOpen(0);
      });
    return () => {
      cancelled = true;
    };
  ...
  const stats = {
    sessions: live?.org.sessions7d ?? 0,
    decisions: live?.org.decisions7d ?? 0,
    blockers: live?.org.open_blockers ?? 0,
    linesAdded: live?.org.lines_added7d ?? 0,
    linesRemoved: live?.org.lines_removed7d ?? 0,
    linesNet: live?.org.lines_net7d ?? 0,
    activeMembers: live?.org.active_members7d ?? 0,
    stuckSessions: live?.org.stuck_sessions ?? 0,
  };

  const orgSeries = live?.daily_activity.map((d) => ({ date: d.date, count: d.count })) ?? [];
  ...
  const topProjects = (live?.top_projects ?? []).map((tp) => {
  ...
  const topContributors = (live?.top_contributors ?? []).map((tc) => {
    ...
    <>
      ...
      <div style={{ padding: "18px 24px 12px", borderBottom: "1px solid var(--border)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {dlqOpen > 0 && (
            <Link
              href="/team/admin/dead-letter"
              title="Open dead-letter events — derive failures the operator should investigate"
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 6,
