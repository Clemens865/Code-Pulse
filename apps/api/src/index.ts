...
import { orgdata } from "./routes/orgdata.js";
...
const server = serve({ fetch: app.fetch, port: env.PORT, hostname: "0.0.0.0" }, (info) => {
  ...
  if (env.NODE_ENV === "production" && env.LOCAL_LOGIN === "true") {
    console.warn(
      "[api] WARNING: LOCAL_LOGIN=true in production — anyone who can reach this API can sign in as any member. Only use behind a trusted network / reverse-proxy auth.",
    );
  }
});
