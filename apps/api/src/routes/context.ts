...
async function resolveProject(
  ...
  // Resolve through project_aliases (mirrors resolveOrCreateProject); a
  // project can carry several canonical_keys from the hook's fallback chain
  // and from historical merges, and the alias index maps all of them to one
  // row. Without this, SessionStart context returns 404 for any merged
  // project's old key — the moat goes silent.
  if (remoteUrl) {
    const { canonicalKey } = normalizeRemoteUrl(remoteUrl);
    if (canonicalKey) {
      const alias = await db.query.projectAliases.findFirst({
        where: (a, { and, eq }) =>
          and(eq(a.orgId, orgId), eq(a.canonicalKey, canonicalKey)),
