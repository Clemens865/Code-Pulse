# Claude Pulse Team — Stack & Deployment
...
│ ~/.claude-pulse-commercial/ (local mode)             │
...
| Sync daemon | Node script bundled into `claude-pulse-team` npm package | Reuses TypeScript types from API contract |
...
| Preview | Per-PR ephemeral environments | `pr-<n>.api.claude-pulse-team.com` + Neon branch DB |
| Staging | Internal dogfood + canary target | `staging.api.claude-pulse-team.com` |
| Production | Customer traffic | `api.claude-pulse-team.com`, `app.claude-pulse-team.com` |
