# Claude Pulse Team — Product Requirements Document
...
**Claude Pulse Team is the middleware that turns each developer's AI session into shared, queryable team knowledge — and feeds that knowledge back into every team member's next AI session as context.**

The single-user Claude Pulse handles individual visibility. Claude Pulse Team adds the collaboration, audit, and reporting layer on top.
...
Alice installs the npm package, runs `claude-pulse-team init`, signs in via OAuth device flow, and the API key is written to `~/.claude-pulse-commercial/config.json`. Hook is wired into her global `~/.claude/settings.json`.
...
Daniel's events haven't synced in 6 hours. Admin UI flags his workstation as stale. Daniel runs `claude-pulse-team doctor`, sees "API key 401 — rotated", fixes it, sync resumes.
...
- Repos with no remote: explicit binding via `claude-pulse-team bind <project-id>`, stored in workstation config — never in the repo.
...
### F9 — Doctor CLI (`claude-pulse-team doctor`)
